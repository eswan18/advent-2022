...
class Board:
    ...
    def move_many(self, count: int, old_column: int, new_column: int):
        '''Move one or more crates from one stack to another.'''
        crates = [self.stacks[old_column].pop() for _ in range(count)]
...
def execute_move(line: str):
    ...
    board.move_many(count, old_column, new_column)
